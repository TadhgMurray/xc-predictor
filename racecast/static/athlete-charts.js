...
/* ------------------------------------------------------------------ *
 *  POOL-VIEW (state owned by scale-view.js, loaded first)
 * ------------------------------------------------------------------ */
 ...
 * alternate value as `vh` (see athlete_chart_data._point). scale-view.js
 * owns the toggle, the .rv table spans and the stored choice; this file
 * only reads the mode and redraws its charts on "rc-scale-change".
 */
function scaleMode() {
  return window.rcScale ? window.rcScale.mode : "pool";
}
...
function initCharts() {
  ...
  /* Redraw every chart when scale-view.js flips the rating scale. It also
     fires once at load when the stored preference is "hs", so the charts
