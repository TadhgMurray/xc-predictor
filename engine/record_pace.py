...
# ★ THE BOARD CANNOT WAIT FOR A REBUILD (owner, 2026-09-14). build_ranking_
#   results applies impossibleRow at step 10, so a corrected curve only
#   reaches the site when the pipeline next runs -- and the rows already
#   stored were gated by the old one. This is the identical rule as a
#   predicate, so a board can refuse a row the build let through.
#
# ! GENERATED FROM _WR_PACE, NEVER TYPED TWICE. The table above is the one
#   definition; this walks it. A record edited there changes both.
#
# ! SEX COMES OFF THE POOL, which is where the boards keep it: 'hs_f' and
#   'hs_f|TF' both end their first segment in _f. ranking_results has no
#   sex column of its own.
def _paceCase(points, dist_sql):
    """Piecewise-linear-in-log-distance record pace, as a SQL CASE."""
    import math
    out = [f"WHEN {dist_sql} <= {points[0][0]} THEN {points[0][1]:.6f}"]
    for (d0, p0), (d1, p1) in zip(points, points[1:]):
        # p0 + (ln d - ln d0)/(ln d1 - ln d0) * (p1 - p0)
        span = math.log(d1) - math.log(d0)
        out.append(
            f"WHEN {dist_sql} <= {d1} THEN {p0:.6f} + "
            f"(ln({dist_sql}) - {math.log(d0):.6f}) / {span:.6f} "
            f"* {(p1 - p0):.6f}")
    out.append(f"ELSE {points[-1][1]:.6f}")
    return "CASE " + " ".join(out) + " END"


def paceFloorSql(time_sql, dist_sql, pool_sql):
    """A predicate that is TRUE for a row this rule calls impossible.

    Every part is a literal built from _WR_PACE and POOL_PACE_FACTOR -- no
    bind parameters, and nothing from a request reaches it, so a caller
    can drop it straight into a WHERE.

    ! SPLIT, NOT LIKE. The level and the sex are segments of the pool
      ('hs_f', 'hs_f|TF'), and matching them with LIKE 'hs\\_%' makes the
      predicate depend on whether the engine treats a backslash in a LIKE
      pattern as an escape -- Postgres does, SQLite does not, and a rule
      that silently means something different in a test harness than in
      production is worse than no rule. split_part means one thing
      everywhere.

    A row missing a time or a distance is NOT impossible: a missing fact is
    not a finding, the same posture impossiblePace takes."""
    bare = f"split_part({pool_sql}, '|', 1)"
    level = f"split_part({bare}, '_', 1)"
    sex = f"split_part({bare}, '_', 2)"
    factor = ("CASE " + " ".join(
        f"WHEN {level} = '{lvl}' THEN {f:.4f}"
        for lvl, f in sorted(POOL_PACE_FACTOR.items())) + " ELSE 1.0 END")
    exempt = ", ".join(f"'{p}'" for p in EXEMPT_POOL_PREFIXES)
    pace = (f"CASE WHEN {sex} = 'f'"
            f" THEN ({_paceCase(_WR_PACE['F'], dist_sql)})"
            f" ELSE ({_paceCase(_WR_PACE['M'], dist_sql)}) END")
    return (f"({time_sql} IS NOT NULL AND {dist_sql} IS NOT NULL"
            f" AND {time_sql} > 0 AND {dist_sql} > 0"
            f" AND {level} NOT IN ({exempt})"
            f" AND {time_sql} / ({dist_sql} / 1000.0)"
            f" < {PACE_FLOOR_SLACK} * ({factor}) * ({pace}))")
